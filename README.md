...
signal. Designed as the substrate [NOX](https://github.com/BonnardValentin) (Rust)
and Telos (TS) plug into; the witness is a versioned JSON wire format so both
languages share one contract.
...
Claude solver → M3 freeze the `warrant/v1` JSON schema + NOX/Rust bridge → M4
